...
public enum SystemPrompt {
    ...
    你还可以获得以下辅助信息来生成更精准的脚本：
    - 抓包数据：用户可能提供通过本地代理捕获的 HTTP/HTTPS 请求（GET/POST 等），包含 URL、请求头、请求体、响应状态和响应体。利用这些数据理解应用的 API 模式、数据格式和存储行为，生成更有针对性的本地分析脚本。
    - 应用数据备份：工具会在脚本执行前自动备份应用数据容器，确保可随时还原。脚本应聚焦于读取和分析，避免不必要的数据写入。
    ...
    5. 如果用户提供了抓包数据，可以分析 API 请求格式、参数结构、响应数据格式，但不得生成伪造或重放网络请求的代码
